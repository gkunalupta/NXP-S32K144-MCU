...
typedef enum PICC_Command{
	...
	typedef enum gb_MFRC522_statusCodes{
		MFRC522_STATUS_OK				,	// Success
		MFRC522_STATUS_ERROR			,	// Error in communication
		MFRC522_STATUS_COLLISION		= 90,	// Collission detected
...
void GB_MFRC522_WriteRegister_Multi(PCD_Register_t GB_reg, byte count, byte *value);
...
gb_MFRC522_statusCodes_t PCD_CalculateCRC(byte *data, byte length,byte *result);	///< In: Pointer to the data to transfer to the FIFO for CRC calculation.\
...
void GB_MFRC522_AnteenaOn(void);
...
gb_MFRC522_statusCodes_t GB_PICC_RequestA(byte * gb_bufferATQ, byte *gb_buffersize);
